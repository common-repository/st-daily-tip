...
License: GPL
*/
define('WP_DAILY_TIP_VERSION', "4.8");
...
function show_daily_tip($atts){
	...
	return add_daily_tip($group,$date,$date_title,$title,$yesterday);
}
function add_daily_tip($grp,$date,$date_title,$title,$yesterday)
{
	...
	$today_tip = select_today_tip($group,$date,$date_title,$title,$yesterday);	
	$today_tip = do_shortcode ($today_tip, true);
	return $today_tip;
}
function show_daily_tip_list($atts){
	extract( shortcode_atts( array(
		'show_last' => '0'
	), $atts ) );
	...
	$sql = "SELECT * FROM $table_name WHERE shown_date != '0000-00-00' AND DATE(shown_date)<DATE('$todate') ORDER BY DATE(shown_date) DESC";
	if($show_last > 0 ){
		$sql .= " LIMIT $show_last";
	}
	$sql .= ";";
	$table_result = $wpdb->get_results($sql);
...
$st_daily_tip_db_ver = "1.7";
$table_suffix = "dailytipdata";

function select_today_tip($group,$date,$date_title,$title,$yesterday = "no"){
	...
	$todate = current_time('mysql',0);
	$today_tip = get_option("st_daily_default_text");
	if($yesterday == "no"){
			...
			if ($tips['tip_title'] != null && $title == "show"){
				if($date=="show"){	
					...
					if($date_title == "show"){
						...
						return "<div class='tip_container'><div class='tip_date'>".$show_date . "</div><div class='tip_title'>" .$tips['tip_title'] . "</div><div class='tip_text'>" .$tips['tip_text'] ."</div></div>";
					}
				}else{
			...
			else
			{
				...
				{	
					...
					if($date_title == "show"){
						...
						return "<div class='tip_container'><div class='tip_text'>" .$tips['tip_text'] . "</div><div class='tip_last_shown'>".$show_date."</div></div>";
					}
				}
				else
				{
		...
		$yesterday =  date('Y-m-d',strtotime($todate." -1 days"));
		$sql = "SELECT * FROM $table_name WHERE DATE(Shown_Date)=DATE('$yesterday') AND group_name='$group';";
		$tips = $wpdb->get_row($sql, ARRAY_A);
		if($tips['tip_text'] != null) {
			if ($tips['tip_title'] != null && $title == "show")
			{
...
function st_daily_tip_install(){
	...
	/* If new installation*/ 
		...
		update_option("st_daily_date_format", 'y-m-d');
		update_option("st_daily_default_text", "No Tips to Display");	
	}
